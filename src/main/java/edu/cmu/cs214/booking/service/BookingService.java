...
public class BookingService {
    ...
    /**
     * Cancels the confirmed booking with {@code bookingId}. If cancellation frees a
     * suitable slot, promotes the earliest eligible user from that room's waitlist.
     */
    public void cancelBooking(String bookingId) {
        store.findBooking(bookingId).ifPresent(booking -> {
            ...
            store.waitlistForRoom(booking.room()).stream()
                    ...
                    .findFirst()
                    .ifPresent(waiter -> {
                        Booking promoted = new Booking("b" + nextBookingSeq++, booking.room(),
                                waiter.user(), waiter.interval());
